...
export class FeedbackService {
  ...
  static async submitFeedback(data: SubmitFeedbackData) {
    ...
    if (
      session.candidateId !== data.fromUserId &&
  ...
  static async getUserFeedback(
    ...
    const where =
      type === 'given' ? { fromUserId: userId } : { toUserId: userId };
   ...
   */
  private static async swapUserRoles(
    candidateId: string,
    ...
    console.log(
      `🔄 Swapped roles: ${candidateId} (CANDIDATE -> INTERVIEWER) <-> ${interviewerId} (INTERVIEWER -> CANDIDATE)`
    );
  }
}
