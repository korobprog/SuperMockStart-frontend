...
async function testCompleteSession() {
  try {
    ...
    console.log('Тестируем завершение сессии...');

    const completeResponse = await makeRequest({
      hostname: 'localhost',
      port: 3001,
      path: '/api/calendar/sessions/test-session-1/complete',
      method: 'PATCH',
      headers: {
    ...
    console.log('Статус ответа:', completeResponse.status);
    console.log(
      'Данные ответа:',
