...
bot.onText(/\/start/, async (msg) => {
    try {
        ...
        const user = msg.from;
        if (!user) {
            await bot.sendMessage(chatId, 'Ошибка: не удалось получить информацию о пользователе');
            return;
        }
        ...
        if (text.startsWith('/start auth_')) {
            ...
            if (currentTime - requestTime > fiveMinutes) {
                await bot.sendMessage(chatId, 'Ссылка для авторизации устарела. Попробуйте еще раз.');
                return;
            }
...
bot.onText(/\/help/, async (msg) => {
    const chatId = msg.chat.id;
    await bot.sendMessage(chatId, `🤖 Помощь по боту SuperMock\n\n` +
        `Команды:\n` +
...
bot.onText(/\/info/, async (msg) => {
    ...
    const user = msg.from;
    if (user) {
        await bot.sendMessage(chatId, `ℹ️ Информация о пользователе:\n\n` +
            `ID: ${user.id}\n` +
